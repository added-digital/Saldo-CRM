...
BEGIN
  ...
  FOR job IN
    ...
    WHERE status IN ('pending', 'processing')
      ...
      AND (
        (payload->>'nightly_chain_id') IS NULL
        OR COALESCE(
          CASE
            WHEN (payload->>'nightly_step_index') ~ '^[0-9]+$' THEN (payload->>'nightly_step_index')::INT
            ELSE NULL
          END,
          0
        ) = 0
        OR EXISTS (
          ...
          WHERE prev.status = 'completed'
            AND prev.payload->>'nightly_chain_id' = payload->>'nightly_chain_id'
            AND CASE
