...
import {
  ...
  REPORTS_V2_FILTERS_STORAGE_KEY,
  turnoverChartConfig,
...
export default function ReportsV2Page() {
  ...
  React.useEffect(() => {
    const saved = parseSavedReportsFilters(
      localStorage.getItem(REPORTS_V2_FILTERS_STORAGE_KEY),
    );
  ...
  const handleResetFilters = React.useCallback(() => {
    localStorage.removeItem(REPORTS_V2_FILTERS_STORAGE_KEY);
...
function renderWorkloadShareCell(percentage: number) {
  ...
  React.useEffect(() => {
    ...
    localStorage.setItem(REPORTS_V2_FILTERS_STORAGE_KEY, JSON.stringify(payload));
  }, [
    loading,
    selectedCustomerId,
    selectedManagerId,
